...
# Note, int this formulae the variable f_cur_i will not be replaced by the current value but by the value computed by the above eulerMethods. Instead, the variable f_i will be replaced by the value which was above f_cur_i
...
while stabilityTest == 1 or i < numberOfSteps:
	...
	try:
		...
		heunApprox_x = heunApprox_x.subs(t_cur, t[i]);
		...
		heunApprox_y = heunApprox_y.subs(t_cur, t[i]);
